...
//exit;
    ...
    public function avAction()
    {
        ...
        if ($this->getRequest()->isPost())
        {
            if (isset($getvalues['addbutton']))
            {
                ...
                else
                {
                    ...
                    $avperson->id = $pid;
                    $avpmap->save($avperson, 'new');
                }
            }
            elseif (isset($getvalues['removebutton']))
            {
                if (isset($getvalues['avpid']))
                {
                    $ids = ($getvalues['avpid']);
                    if (count($ids) > 0)
                    {
                        ...
                        foreach ($ids as $id)
                        {
                            ...
                            $avperson = $avpmap->find($id);
                            $avpmap->delete($id);
                            $this->view->message[] = "$name removed from the AV list.";
                        }
                    }
                    else
              ...
              else
              {
                  ...
                  else
                  {
                      ...
                      foreach($dates as $date)
                      {
                          ...
                          $avstatus = $avsmap->fetchWhere($where);
                          if (count($avstatus) == 0)
                          {
                            /* no old entry */
                              $avstatus = new Application_Model_AVStatus();
                              $avstatus->peopleid = $pid;
                              $avstatus->dateid = $dateid;
                              $avstatus->early = $early;
                              $avstatus->late = $late;
                              $avsmap->save($avstatus);
                          }
                          else
                          {
                              $avstatus = current($avstatus);
                              if ($early <> ' ')
                                  $avstatus->early = $early;
                              if ($late <> ' ')
                                  $avstatus->late = $late;
        ...
        $avpeopleids = $avpmap->fetchAll();
        $initialsBoxes = '';
        $initialsList = '';
        foreach ($avpeopleids as $id)
        {
            if ($id->id == $pid)
                $checked = ' checked';
            else
      	...
      	foreach($gridentries as $gridentry) 
        {
            if ($countDates++ > 15)
        		...
        		$dateID = $gridentry->id;
            $peoparray = array();
            foreach ($avpeopleids as $personid)
            {
                $id = $personid->id;
                $where = array(
                    array('dateid' , ' = ', $dateID),
                ...
                $starray = $avsmap->fetchWhere($where);
                if (count($starray) == 1) 
                {
            ...
            $table[] = array($dateID, $gridentry->servicedate, $peoparray);
        }
        ...
        foreach ($avpeopleids as $id)
        {
            ...
            $name = $person['firstname'] . ' ' . $person['lastname'];
            $initials = $person['firstname'][0] . substr($person['lastname'], 0, 2);
            $people[] = array($id->id, $name, $initials);
      	}
    ...
    public function sextonsAction()
    {
        ...
        $sextonspeople = $sextonspmap->fetchAll();
      	
        $statusCodes['away'] = 'x';
      	$statusCodes['clear'] = '&nbsp;';
      	$statusCodes['sextonsailable'] = '&nbsp;';
      	$statusCodes['scheduled'] = 'S';
        ...
        if ($this->getRequest()->isPost())
        {
            if (isset($getvalues['addbutton']))
            {
                ...
                else
                {
                    ...
                    $sextonsperson->id = $pid;
                    $sextonspmap->save($sextonsperson, 'new');
                    $this->view->message = "Created new Sextons people entry for $name.";
                    $pid = 0;
                }
            }
            elseif (isset($getvalues['removebutton']))
            {
                if (isset($getvalues['pid']))
                {
                    $ids = ($getvalues['pid']);
                    if (count($ids) > 0)
                    {
                        ...
                        foreach ($ids as $id)
                        {
                            $person = $peoplemap->find($id);
                            $name = $person['firstname'] . ' ' . $person['lastname'];
                            $sextonsperson = $sextonspmap->find($id);
                            $sextonspmap->delete($id);
                            $this->view->message[] = "$name removed from the sextons list.";
                        }
                    }
                }
                else
                    $this->view->message = "No ID selected to remove.";

            
            }
            elseif (isset($getvalues['status']))
            {
                if (isset($getvalues['pid']))
                {
                    $ids = ($getvalues['pid']);
                    ...
                    $this->view->message = "No ID selected to set status.";
            }
            ...
            if ($pid <> 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        if (isset($getvalues['status']))
                        {
                            $status = strtolower($getvalues['status']);
                        }  
                        ...
                        foreach($dates as $date)
                        {
                            ...
                            $sextonsstatus = $sextonssmap->fetchWhere($where);
                            if (count($sextonsstatus) == 0)
                            {
                              /* no old entry */
                                $sextonsstatus = new Application_Model_SextonsStatus();
                                $sextonsstatus->peopleid = $pid;
                                $sextonsstatus->dateid = $dateid;
                                $sextonsstatus->status = $status;
                                $sextonssmap->save($sextonsstatus);
                            }
                            else
                            {
                                $sextonsstatus = current($sextonsstatus);
                                $sextonsstatus->status = $status;
                                $sextonssmap->save($sextonsstatus);
        ...
        $sextons = array();
        foreach ($sextonspeople as $sexton)
        {
             $peep = $peoplemap->find($sexton->id);
             $sextons[] = array($peep['id'], $peep['lastname'] . $peep['firstname']);
        }
        function custom_sort($a,$b) {
            return strcmp($a[1], $b[1]);
        }
        usort($sextons, "custom_sort");        
        $initialsBoxes = '';
        $initialsList = '';
        foreach ($sextons as $sexton)
        {
            ...
            $initials = $person['firstname'][0] . $person['lastname'][0];
        		$initialsList = $initialsList . '<th>' . $initials;
            $initialsBoxes = $initialsBoxes . '<td><input type="checkbox" name="pid[]" value=' . $id . $checked . '>';
      	}
        $this->view->initialsBoxes = $initialsBoxes;
        $this->view->initialsList = $initialsList;
      	...
      	foreach($gridentries as $gridentry) 
        {
            ...
            foreach ($sextons as $sexton)
            {
                ...
                $starray = $sextonssmap->fetchWhere($where);
                if (count($starray) == 1) 
                {
                    $theStatus = current($starray)->status;
            		    $column = $statusCodes[$theStatus];
            		}
                else 
                    $column = '&nbsp;';
                $peoparray[] = $column;
        ...
        $sextonspeopleids = $sextonspmap->fetchAll();
      	$initialsTable = array();
      	$peopleTable = array();
        $people = array();
        foreach ($sextons as $sexton)
        {
            $person = $peoplemap->find($sexton[0]);
            $name = $person['firstname'] . ' ' . $person['lastname'];
            $initials = $person['firstname'][0] . $person['lastname'][0];
            $people[] = array($sexton[0], $name, $initials);
      	}
  ...
  /* commit button */
    ...
    public function welcomingAction()
    {
        ...
        if ($this->getRequest()->isPost())
        {
            if (isset($getvalues['addbutton']))
            {
                ...
                else
                {
                    ...
                    $wlperson->id = $pid;
                    $wlpmap->save($wlperson, 'new');
                }
            }
            elseif (isset($getvalues['removebutton']))
            {
                if (isset($getvalues['wlpid']))
                {
                    $ids = ($getvalues['wlpid']);
                    if (count($ids) > 0)
                    {
                        ...
                        foreach ($ids as $id)
                        {
                            $person = $peoplemap->find($id);
                            $name = $person['firstname'] . ' ' . $person['lastname'];
                            $wlperson = $wlpmap->find($id);
                            $wlpmap->delete($id);
                            $this->view->message[] = "$name removed from the Welcoming list.";
                        }
                    }
                    else
                        $this->view->message = "No ID selected.";
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        foreach($dates as $date)
                        {
                            ...
                            $wlstatus = $wlsmap->fetchWhere($where);
                            if (count($wlstatus) == 0)
                            {
                              /* no old entry */
                                $wlstatus = new Application_Model_WelcomingStatus();
                                $wlstatus->peopleid = $pid;
                                $wlstatus->dateid = $dateid;
                                $wlstatus->early = $early;
                                $wlstatus->late = $late;
                                $wlsmap->save($wlstatus);
                            }
                            else
                            {
                                $wlstatus = current($wlstatus);
                                if ($early <> ' ')
                                    $wlstatus->early = $early;
                                if ($late <> ' ')
                                    $wlstatus->late = $late;
        ...
        $wlpeopleids = $wlpmap->fetchAll();
        $initialsBoxes = '';
        $initialsList = '';
        foreach ($wlpeopleids as $id)
        {
            if ($id->id == $pid)
                $checked = ' checked';
            else
      	...
      	foreach($gridentries as $gridentry) 
        {
            if ($countDates++ > 15)
        		...
        		$dateID = $gridentry->id;
            $peoparray = array();
            foreach ($wlpeopleids as $personid)
            {
                $id = $personid->id;
                $where = array(
                    array('dateid' , ' = ', $dateID),
                ...
                $starray = $wlsmap->fetchWhere($where);
                if (count($starray) == 1) 
                {
                    $theStatus = current($starray);
            		    $column = $statusCodes[$theStatus->Early] . '|' .
        ...
        if ($this->getRequest()->isPost())
        {
            if (isset($getvalues['addbutton']))
            {
                ...
                else
                {
                    ...
                    $emperson->id = $pid;
                    $empmap->save($emperson, 'new');
                }
            }
            elseif (isset($getvalues['removebutton']))
            {
//var_dump($getvalues);
                if (isset($getvalues['empid']))
                {
                    $ids = ($getvalues['empid']);
                    if (count($ids) > 0)
                    {
                        ...
                        foreach ($ids as $id)
                        {
                            $person = $peoplemap->find($id);
                            $name = $person['firstname'] . ' ' . $person['lastname'];
                            $emperson = $empmap->find($id);
                            $empmap->delete($id);
                            $this->view->message[] = "$name removed from the EMAIL list.";
                        }
                    }
                    else
                        $this->view->message = "No ID selected.";
              ...
              else
              {
                  ...
                  else
                  {
                      ...
                      foreach($dates as $date)
                      {
                          ...
                          $emstatus = $emsmap->fetchWhere($where);
                          if (count($emstatus) == 0)
                          {
                            /* no old entry */
                              $emstatus = new Application_Model_EMStatus();
                              $emstatus->peopleid = $pid;
                              $emstatus->dateid = $dateid;
                              $emstatus->early = $early;
                              $emsmap->save($emstatus);
                          }
                          else
                          {
                              $emstatus = current($emstatus);
                              if ($early <> ' ')
                                  $emstatus->early = $early;
                              if ($early <> 'available')
                                  $emsmap->save($emstatus);
        ...
        $empeopleids = $empmap->fetchAll();
        $initialsBoxes = '';
        $initialsList = '';
        foreach ($empeopleids as $id)
        {
            if ($id->id == $pid)
                $checked = ' checked';
            else
      	...
      	foreach($gridentries as $gridentry) 
        {
            if ($countDates++ > 15)
        		...
        		$dateID = $gridentry->id;
            $peoparray = array();
            foreach ($empeopleids as $personid)
            {
                $id = $personid->id;
                $where = array(
                    array('dateid' , ' = ', $dateID),
                ...
                $starray = $emsmap->fetchWhere($where);
                if (count($starray) == 1) 
                {
                    $theStatus = current($starray);
            		    $column = $statusCodes[$theStatus->Early];
