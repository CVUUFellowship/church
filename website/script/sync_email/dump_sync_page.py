...
def build_page(outf, panes):
  print >>outf, page_head
  cur_head = first_head
  for pane in panes:
    print >>outf, cur_head.format(pane.divid, pane.title)
    cur_head = later_head
  print >>outf, tabs_start
  pane_arg = ' in active'
  for pane in panes:
    print >>outf, pane_open.format(pane.divid, pane_arg)
    pane_arg = ''
    probset = ProblemSet()
    for table in pane.tables:
      print >>outf, table_head.format(table.tableId)
      print >>outf, EmailRow.row_header(use_groups=table.use_groups)
      print >>outf, table_body
      for row in table.emailRows:
        print >>outf, '  <tr>'
        print >>outf, row.row_contents(probset, use_groups=table.use_groups).encode('utf-8')
        print >>outf, '  </tr>'
      print >>outf, table_done
    print >>outf, '<p>'
    just_announcements = probset.no_announcements - probset.no_chimps
    if just_announcements:
      print >>outf, "<strong>addresses not receiving announcements: </strong>%s<br/>" % (', '.join(just_announcements))
    if probset.no_chimps:
      print >>outf, "<strong>addresses not receiving any mailchimp: </strong>%s<br/>" % (', '.join(probset.no_chimps))
    if probset.neighborhood:
      print >>outf, "<strong>addresses not on neighborhood list: </strong>%s<br/>" % (', '.join(probset.neighborhood))
    print >>outf, '</p>'
    print >>outf, pane_close
  print >>outf, '<p>Finished rebuild at {0}</p>'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
  print >>outf, finish_page
