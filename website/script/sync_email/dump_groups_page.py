  ...
  with open(build_name, "w") as outf:
    ...
    for group in groups:
      ...
      if now - last_log > datetime.timedelta(seconds=2):
        ...
        last_log = now
      print >>outf, '<b>{0}</b>:<br/>'.format(group.group_email)
      for member in group.members:
        if member.email in db_members:
          ...
          name = ''
        print >>outf, '&nbsp;&nbsp;&nbsp;{0}({1})<br/>'.format(name, member.email)
      print >>outf, '<br/>'
    print >>outf, '<p>Finished building at {when}</p>'.format(when=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
