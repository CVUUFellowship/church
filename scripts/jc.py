...
def make_group(service, groupId, settings):
...
def main(argv):
  ...
  parser.add_option('--description',
                    help='group description', default='')
  ...
  settings = {'description': options.description,
              'name': options.name if options.name else options.groupId.split('@', 1)[0],
              'email': options.groupId }
  ...
  make_group(service=service, groupId=options.groupId, settings=settings)
