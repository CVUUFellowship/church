...
class MailchimpReceiver(object):
  ...
  def __init__(self, email, fname, lname, mail_segments):
    ...
    self.seg_sum = ','.join(MailchimpReceiver.seg_map[seg] for seg in mail_segments if seg)
...
def get_list_id():
    ...
    if True:
        ...
        api_fields = json.dumps({'apikey':mailchimp_key, 'filters':{}, 'start':0, 'limit':100}).encode('utf-8')
        req = Request('https://%s.api.mailchimp.com/2.0/lists/list.format' % (domain), api_fields)
...
["ussb@ussb.org","Unitarian Society of Santa Barbara","","Monthly Newsletter","No Neighborhood Emails Please",2,"",null,"2014-11-03 04:29:37","108.184.30.134",null,null,null,null,null,null,null,"2014-11-03 04:29:37","222637881","fbd96ba9d4",null]
    ...
    domain = mailchimp_key.rsplit('-', 1)[1]
    list_id = get_list_id()
    api_fields = urlencode({'apikey':mailchimp_key, 'id':list_id})
    req = Request(('https://%s.api.mailchimp.com/export/1.0/list?' % (domain))+api_fields)
    resp_data = get_response_data(req)
    resp_buf = StringIO(resp_data.decode('utf-8'))
    lines = [line for line in [lin.strip() for lin in resp_buf] if line != '']
...
def fetch_list_first():
    ...
    {"total":298,"data":[
      {"email":"redacted@redacted.com","id":"0126867a95",
       ...
       "merges":{"EMAIL":"redacted@redacted.com","FNAME":"Susan","LNAME":"Dixon",
      ...
      {"email":"redacted2@redacted2.com","id":"0167dfbe85",
       ...
       "list_name":"Conejo Valley UU Fellowship Members and Friends",
       "merges":{"EMAIL":"redacted2@redacted2.com","FNAME":"Judy","LNAME":"Anderson",
       "GROUPINGS":[{"id":13233,"name":"Announcements","form_field":"checkboxes",
          "groups":[{"name":"Weekly Announcements","interested":true},
                    {"name":"Monthly Newsletter","interested":true},
                    {"name":"Auction Updates","interested":true}]},
    ...
    domain = mailchimp_key.rsplit('-', 1)[1]
    while not done:
        api_fields = json.dumps({'apikey':mailchimp_key, 'id':list_id, 'opts':{'limit':step, 'start':start}}).encode('utf-8')
        req = Request('https://%s.api.mailchimp.com/2.0/lists/members.format' % (domain), api_fields)
        resp_data = get_response_data(req)
        resp_json = json.loads(resp_data)
...
def fetch_activity(emails):
    ...
    {"success_count":3,"error_count":0,"errors":[],"data":[
        {"email":{"email":"redacted3@redacted3.net"},
        "activity":[
            {"action":"open",
    ...
    for start, stop in zip(range(0, len(emails), step), range(step, len(emails), step) + [len(emails)]):
        ...
        api_fields = json.dumps({'apikey':mailchimp_key, 'id':list_id, 'emails':use_em}).encode('utf-8')
        req = Request('https://%s.api.mailchimp.com/2.0/lists/member-activity.format' % (domain), api_fields)
        resp_data = get_response_data(req)
        resp_json = json.loads(resp_data)
